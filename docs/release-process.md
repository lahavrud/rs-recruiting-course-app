...
2. **Click "Deploy Staging"** (`deploy-staging.yml`, optionally pinning a `ref` other than `main`). It computes the next release-candidate version from the commits since the last final tag and pushes `vX.Y.Z-rc.N`. That tag push triggers `release.yml`'s `build-rc` job, which builds and pushes the image.
